...
## Ollama Model Drift from Stored Config - 2026-03-29
**Severity**: High  
**Context**: Extension requests alternated between expected model and stale deepseek model in background/content flows.  
**Symptoms**: 
- Network payload showed `model: "deepseek-r1:14b"` for `/v1/chat/completions` even after code default moved to `llama3.1`
- Some APIs used config-driven model while other paths had hardcoded model literals

**Root Cause**: 
Persisted `ollamaConfig.chatModel` in browser storage still contained legacy deepseek value; config-driven callers honored it while other callers used hardcoded literals, creating inconsistent runtime behavior.

**Solution**: 
1. Standardized callers to resolve model via `getOllamaConfig()`
2. Added config migration in `getOllamaConfig()` to normalize legacy deepseek chat models to `llama3.1`
3. Persist migrated config back to storage so subsequent runs are consistent

**Prevention**: 
- Never hardcode chat model in service calls; always use centralized config resolver
- Add migration guards whenever default model changes

**Related Files**: 
- `apps/extension-chrome/src/shared/ollama-config.ts`
- `apps/extension-chrome/src/shared/ollama-client.ts`
- `apps/extension-chrome/src/shared/ollama-service.ts`
- `apps/extension-chrome/src/shared/text-transform-service.ts`
- `apps/extension-chrome/src/shared/field-validator.ts`
