...
import { applyGraphEnhancement, applyAIOptionFallback } from './shared/autofill';
...
async function postFillRescan(profile: UserProfile): Promise<void> {
  ...
  let newMappings = generateFillMappings(newFields, profile);
  try {
    const hydratedNewFields = await hydrateOptionsForFallback(newFields, newMappings);
    const fallbackResult = await applyAIOptionFallback(hydratedNewFields, newMappings, profile);
    newMappings = fallbackResult.mappings;
    info('[AI Fallback] Rescan summary:', fallbackResult.diagnostics);
  } catch (err) {
    warn('[AI Fallback] Rescan fallback failed:', err);
  }
  if (newMappings.length === 0) {
...
/**
 * Attempt to hydrate options for unresolved option-like fields before AI fallback.
 * Dynamic ATS dropdowns often render options only after focus/click/open.
 */
async function hydrateOptionsForFallback(
  schema: FieldSchema[],
  existingMappings: Array<{ selector: string }>
): Promise<FieldSchema[]> {
  const filled = new Set(existingMappings.map(m => m.selector));
  const candidates = schema.filter(field => {
    if (filled.has(field.selector)) return false;
    if (field.fillEligible === false || field.trustTier === 3) return false;

    const type = (field.type || '').toLowerCase();
    const hasOptionLikeType = type === 'autocomplete' || type === 'select' || type === 'select-one' || type === 'radio';

    const options = ('options' in field && Array.isArray((field as any).options))
      ? (field as any).options as string[]
      : [];
    const hasRealOptions = options.some(o => {
      const l = String(o || '').trim().toLowerCase();
      return l && l !== 'no options' && l !== 'select...' && l !== 'select';
    });
    const hasRadioOptions = Array.isArray(field.radioOptions) && field.radioOptions.length > 1;

    return hasOptionLikeType && !hasRealOptions && !hasRadioOptions;
  });

  if (candidates.length === 0) return schema;
  ...
  try {
    ...
    if (forms.length > 0) {
      for (const form of forms) {
        refreshed.push(...extractFormSchema(form));
      }
    } else {
      refreshed = extractFormSchema(document);
    }
  } catch (err) {
