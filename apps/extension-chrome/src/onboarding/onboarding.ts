...
function showErrorDetails(error: string, details?: any): void {
  ...
  if (errorKind === 'model-not-found') {
    html += `
      ...
      <pre style="background:#1e2a3a;color:#a3e635;padding:10px 12px;border-radius:6px;font-size:12px;margin:8px 0;white-space:pre-wrap;word-break:break-all;">ollama pull llama3.1</pre>
    ...
    html += `
      ...
      <pre style="background:#1e2a3a;color:#a3e635;padding:10px 12px;border-radius:6px;font-size:12px;margin:8px 0;white-space:pre-wrap;word-break:break-all;">OLLAMA_ORIGINS='chrome-extension://*' ollama serve</pre>
      ...
      <pre style="background:#1e2a3a;color:#a3e635;padding:10px 12px;border-radius:6px;font-size:12px;margin:8px 0;white-space:pre-wrap;word-break:break-all;">echo 'export OLLAMA_ORIGINS="chrome-extension://*"' >> ~/.zshrc &amp;&amp; source ~/.zshrc</pre>
    ...
    html += `
      ...
      <ul>
        ...
        <li>If you see "403 / Forbidden", restart Ollama with:
          <pre style="background:#1e2a3a;color:#a3e635;padding:8px 10px;border-radius:5px;font-size:12px;margin:6px 0;">OLLAMA_ORIGINS='chrome-extension://*' ollama serve</pre>
        </li>
...
function stripAnsi(text: string): string {
  // Remove ANSI escape sequences (colour codes, cursor movement, progress bars)
...
function appendToLog(logEl: HTMLElement, rawText: string, isError = false): void {
  ...
  if (!text) return; // skip blank / escape-only lines
  const line = document.createElement('div');
  line.textContent = text;
...
function setupSkillsEventListeners(): void {
  ...
  // Remove existing chips (event delegation)
  container.addEventListener('click', (e) => {
  ...
  if (input) {
    ...
    // Also handle paste: split by comma/newline and add multiple chips
...
function collectProfileFromForm(): UserProfile | null {
  ...
  if (skillsList) {
    ...
    // Also capture anything currently typed in the input but not yet committed
  ...
  if (parseBtn) {
    parseBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        // Show review step — reset button first so re-entry is clean
        parseBtn.disabled = false;
