...
const LEGACY_CHAT_MODELS = new Set(['deepseek-r1', 'deepseek-r1:14b']);

function normalizeChatModel(model: string | undefined): string {
  const m = String(model || '').trim();
  if (!m) return DEFAULT_OLLAMA_CONFIG.chatModel;
  if (LEGACY_CHAT_MODELS.has(m)) return DEFAULT_OLLAMA_CONFIG.chatModel;
  return m;
}
...
export async function getOllamaConfig(): Promise<OllamaConfig> {
  try {
    ...
    if (stored && typeof stored === 'object') {
      const merged = { ...DEFAULT_OLLAMA_CONFIG, ...stored } as OllamaConfig;
      const normalized = { ...merged, chatModel: normalizeChatModel(merged.chatModel) };
      // Persist migration so every caller sees the same model next load.
      if (normalized.chatModel !== merged.chatModel) {
        await browser.storage.local.set({ [STORAGE_KEY]: normalized });
        console.info('[OllamaConfig] Migrated legacy chat model to', normalized.chatModel);
      }
      return normalized;
    }
  } catch (err) {
