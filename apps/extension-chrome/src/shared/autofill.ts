 ...
 *
 * Strategy:
 * - Only runs for fields still unfilled after profile/RL/graph passes.
 * - Requires concrete options (radio options or select/combobox options).
 * - Sends question + options + full profile to Ollama and accepts ONLY exact option matches.
 */
export async function applyAIOptionFallback(
  schema: FieldSchema[],
  existingMappings: FillMapping[],
  profile: UserProfile
): Promise<{
  ...
  diagnostics: {
    ...
    filled: number;
    skipped: Record<
      'already_filled' | 'untrusted' | 'missing_question' | 'insufficient_options' | 'model_error' | 'non_exact_match',
      number
    >;
  };
}> {
  const filledSelectors = new Set(existingMappings.map(m => m.selector));
  const enhanced = [...existingMappings];
  const mappingIndexBySelector = new Map(existingMappings.map((m, idx) => [m.selector, idx]));
  const diagnostics = {
    attempted: 0,
    filled: 0,
    skipped: {
  ...
  const model = config.chatModel || 'llama3.1';

  const getFieldOptions = (field: FieldSchema): string[] => {
    const optionSet = new Set<string>();

    if (Array.isArray(field.radioOptions)) {
    ...
    if ('options' in field && Array.isArray((field as { options?: string[] }).options)) {
    ...
    return Array.from(optionSet).filter(o => {
      const l = o.toLowerCase();
      return l !== 'no options' && l !== 'select...' && l !== 'select';
    });
  };

  const parseSelectedOption = (raw: string): string | null => {
    const cleaned = raw.trim().replace(/```json\s*/g, '').replace(/```\s*/g, '').trim();
    try {
      const parsed = JSON.parse(cleaned) as { selectedOption?: string };
      if (typeof parsed.selectedOption === 'string') return parsed.selectedOption.trim();
    } catch {
      // Fall back to plain-text exact output
    }
    return cleaned;
  };

  const shouldReconsiderFilledField = (
    field: FieldSchema,
    ...
    if (!v || vLower === 'select...' || vLower === 'select' || vLower === 'no options' || vLower === 'unknown') {
      return true;
    }
  ...
  for (const field of schema) {
    if (field.fillEligible === false || field.trustTier === 3) {
      diagnostics.skipped.untrusted++;
      console.log('[Autofill/AI-Fallback] fallback_skipped_reason=untrusted', field.selector);
      continue;
    }

    const question = (field.label || field.name || field.id || '').trim();
    if (!question || question.length < 3) {
      diagnostics.skipped.missing_question++;
      console.log('[Autofill/AI-Fallback] fallback_skipped_reason=missing_question', field.selector);
      continue;
    }

    const options = getFieldOptions(field);
    if (options.length < 2) {
      diagnostics.skipped.insufficient_options++;
      console.log('[Autofill/AI-Fallback] fallback_skipped_reason=insufficient_options', {
        selector: field.selector,
        question,
        optionsCount: options.length,
      });
      continue; // No usable options to constrain model output
    }

    if (filledSelectors.has(field.selector)) {
      const idx = mappingIndexBySelector.get(field.selector);
      const currentVal = idx !== undefined ? enhanced[idx]?.value : undefined;
      if (idx === undefined || currentVal === undefined || !shouldReconsiderFilledField(field, currentVal, options)) {
        diagnostics.skipped.already_filled++;
        console.log('[Autofill/AI-Fallback] fallback_skipped_reason=already_filled', field.selector);
        continue;
      }
      console.log('[Autofill/AI-Fallback] reconsidering_filled_field', {
        selector: field.selector,
        question,
    ...
    console.log('[Autofill/AI-Fallback] fallback_attempted', {
      selector: field.selector,
      question,
      optionsCount: options.length,
    });
    ...
    const prompt = [
      'You are a job application answer selector.',
      'Choose exactly one option from the provided options for the given question using the candidate profile.',
      'Return JSON only: {"selectedOption":"<exact option text>"}',
      'Rules:',
      '1) selectedOption must be exactly one of the provided options (character-for-character).',
      '2) Do not invent or paraphrase options.',
      '3) If uncertain, choose the safest "Decline/Prefer not to say/No" style option from the list when available.',
      '',
      `Question: ${question}`,
    ...
    try {
      ...
      const existingIdx = mappingIndexBySelector.get(field.selector);
      if (existingIdx !== undefined) {
        enhanced[existingIdx] = { selector: field.selector, value: exactMatch };
      } else {
        mappingIndexBySelector.set(field.selector, enhanced.length);
        enhanced.push({ selector: field.selector, value: exactMatch });
      }
      filledSelectors.add(field.selector);
      diagnostics.filled++;
      console.log('[Autofill/AI-Fallback] fallback_selected_option', {
        selector: field.selector,
        question,
        selected: exactMatch,
      });
      console.log(`[Autofill/AI-Fallback] Filled "${question}" with "${exactMatch}"`);
    } catch (err) {
      diagnostics.skipped.model_error++;
      console.warn('[Autofill/AI-Fallback] Model selection failed:', err);
    }
  }

  console.log('[Autofill/AI-Fallback] summary', diagnostics);
  return { mappings: enhanced, diagnostics };
}
